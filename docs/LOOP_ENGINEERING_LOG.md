...
## پس از فاز ۱۲: تست واقعی روی Supabase (از طریق MCP)

بعد از تکمیل Milestone 1، دسترسی MCP به یک پروژه Supabase واقعی فراهم
شد و هر ۵ مایگریشن روی آن اجرا شد. Security Advisor خودکار Supabase یک
**باگ امنیتی واقعی** پیدا کرد که هیچ بازبینی استاتیکی نمی‌توانست پیدا
کند: `purge_expired_trash()` بدون هیچ محدودیت دسترسی‌ای در دسترس همه‌ی
کاربران بود. شرح کامل، رفع مشکل (مایگریشن‌های ۰۰۶ و ۰۰۷)، و درسی که حین
رفع یاد گرفته شد (REVOKE از نقش‌های خاص کافی نیست، باید از `PUBLIC` هم
باشد) در `docs/IMPLEMENTATION_SUMMARY.md` (بخش «تست واقعی روی Supabase»)
آمده.

---

## تست واقعی روی GitHub Actions

با دسترسی به GitHub API (توکن با پرمیشن `Actions: Read`)، معلوم شد
**همه‌ی ۱۰ اجرای CI از فاز ۴ به بعد fail شده بودن** — چیزی که بازبینی
استاتیک من هرگز نمی‌تونست بگیره چون کاملاً بیرون از دامنه‌ی کد Dart/SQL
بود: خود پیکربندی `ci.yml`.

### مشکل

مرحله‌ی `subosito/flutter-action@v2` در عرض ۶ ثانیه fail می‌شد (خیلی
سریع‌تر از یک دانلود واقعی SDK) — نشانه‌ی یک خطای اعتبارسنجی ورودی، نه
خطای شبکه. با جست‌وجوی مستندات رسمی action مشخص شد: وقتی هم `channel`
و هم `flutter-version` روی `"stable"` تنظیم بشن، action سعی می‌کنه
`"stable"` رو به‌عنوان یک شماره نسخه‌ی واقعی روی کانال stable resolve
کنه که وجود نداره → خطای فوری.

`ci.yml` من دقیقاً همین دو خط رو با هم داشت (از حافظه نوشته شده بود،
بدون تأیید از مستندات رسمی، چون این محیط دسترسی به GitHub Actions
Marketplace نداره).

### رفع

خط `flutter-version: "stable"` حذف شد؛ فقط `channel: "stable"` باقی
موند (که خودش کافیه و آخرین نسخه‌ی پایدار رو نصب می‌کنه).

### درس کلی

این دومین مورد (بعد از باگ امنیتی Supabase) از یک الگوی مشخص است:
بازبینی استاتیک من روی **منطق کد** (Dart/SQL که خودم می‌نویسم) قابل
اتکاست، اما روی **پیکربندی ابزارهای شخص ثالث** (GitHub Actions، npm
packages، و غیره) که باید طبق مستندات رسمی و به‌روز تنظیم بشن، نه —
چون این محیط به مستندات زنده یا اجرای واقعی آن ابزارها دسترسی ندارد.
...
## Milestone 2 — فاز ۱: Product Catalog

مایگریشن `008_product_catalog.sql` روی پروژه واقعی اجرا شد: `brands`,
`products`, `product_aliases`, `user_product_history` + RPC های
`get_or_create_product`, `record_product_purchase`,
`toggle_favorite_product` + RLS.

### دو یافته‌ی امنیتی جدید (بلافاصله با `get_advisors` چک شد)

1. **باگ امنیتی بحرانی دیگر**: `mark_item_purchased()` (از فاز ۶، قبل
   از اینکه دسترسی MCP وجود داشته باشه) اصلاً auth check نداشت. چون
   `security definer` بود، RLS جدول `shopping_items` رو کامل دور می‌زد
   — یعنی **هر کاربر لاگین‌کرده می‌تونست با حدس‌زدن یه UUID، آیتم هر
   خانواده‌ی دیگه‌ای رو تیک بزنه یا قیمت دلخواه براش ثبت کنه**. رفع شد:
   `mark_item_purchased` حالا دقیقاً همون چک مالکیت/عضویت خانواده رو
   داره که خود RLS جدول `shopping_items` داره (۰۰۹).
2. توابع خانواده/لیست که در فازهای ۴-۶ (قبل از دسترسی واقعی) نوشته شده
   بودن، هیچ‌وقت صریحاً از `anon` revoke نشده بودن — فقط به auth.uid()
   null-check داخلی متکی بودن. با اینکه اکثرشون به‌طور اتفاقی امن بودن
   (constraint های NOT NULL جلوشونو می‌گرفت)، صریح revoke شدن (۰۰۹/۰۱۰).

### درس تکراری (این بار واقعاً یاد گرفته شد)

موقع نوشتن `009_security_hardening_v3.sql` **دقیقاً همون اشتباه فاز ۸
تکرار شد**: فقط `revoke ... from anon` نوشتم، نه `from public`. با چک
فوری `get_advisors` بلافاصله بعد از اجرا (نه دیر) همون ۹ تابع هنوز
flag می‌شدن. رفعش در `010_security_hardening_v4.sql`. **نتیجه‌ی عملی
این تکرار**: از این به بعد قانون ثابت شد — هر `REVOKE EXECUTE` روی یک
تابع باید حتماً `FROM PUBLIC` باشه، نه فقط نقش‌های خاص Supabase
(`anon`/`authenticated`).

### وضعیت نهایی

بعد از ۰۰۹ و ۰۱۰، فقط ۱۲ تابع RPC که *عمداً* برای کاربران لاگین‌شده
عمومی‌ان (با auth check داخلی خودشون) در گزارش امنیتی باقی موندن —
دقیقاً همونی که انتظار می‌رفت.

### آنچه هنوز در Dart پیاده نشده

لایه‌ی Domain/Data/Presentation فیچر `catalog` (که پوشه‌هاش ساخته شدن)
هنوز خالیه. طبق تفکیک خود `36_MILESTONE_2.md`، وصل‌کردن این کاتالوگ به
UI جستجو کار **فاز ۲ (Smart Search)** است، نه فاز ۱ («Deliverable:
Structured product database» — یعنی خود دیتابیس، نه لزوماً UI). این
تفکیک عمدی است، نه جا افتادن کار.
